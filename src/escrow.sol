...
contract EscrowByAgentV2 is Ownable, ReentrancyGuard, IEscrowByAgent {
    ...
    function release(
        ...
        if (pool.token != address(0x0)) {
            IERC20(pool.token).transfer(
                pool.recipient,
                pool.amount - fee - agentFee
            );
            IERC20(pool.token).transfer(owner(), fee);
            IERC20(pool.token).transfer(pool.agent, agentFee);
        } else {
    ...
    function cancel(
        ...
        if (pool.token != address(0x0)) {
            IERC20(pool.token).transfer(pool.sender, pool.amount);
        } else {
